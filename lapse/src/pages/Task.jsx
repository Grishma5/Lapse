...
const TaskManager = () => {
  ...
  const columns = [
    { id: 'To-Do', title: 'To Do', color: 'from-pink-400 to-rose-400' },
    { id: 'In-Progress', title: 'In Progress', color: 'from-purple-400 to-violet-400' },
    { id: 'Done', title: 'Done', color: 'from-indigo-400 to-blue-400' }
  ];
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-4xl font-bold text-gray-800 mb-2 font-cute">
                Task <span className="bg-gradient-to-r from-pink-400 to-purple-400 bg-clip-text text-transparent">Board</span>
              </h1>
              <p className="text-gray-600 font-cute">Organize and track your tasks with ease</p>
            </div>
            <button
              onClick={() => openModal()}
              className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-pink-400 to-purple-400 text-white rounded-full hover:from-pink-500 hover:to-purple-500 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 font-cute"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {columns.map(column => (
            <div
              key={column.id}
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-pink-100 overflow-hidden"
              ...
              <div className="p-4 space-y-4 min-h-[500px]">
                {getTasksByStatus(column.id).map(task => (
                  <div
                    ...
                    className="bg-gradient-to-br from-pink-50 to-purple-50 rounded-xl p-4 cursor-move hover:shadow-lg transition-all duration-200 border border-pink-100 hover:border-purple-200 group"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center">
                        <GripVertical className="w-4 h-4 text-gray-400 mr-2 opacity-0 group-hover:opacity-100 transition-opacity" />
                        <h4 className="font-semibold text-gray-800 text-sm leading-tight font-cute">{task.title}</h4>
                      </div>
                    ...
                    {task.description && (
                      <p className="text-gray-600 text-sm mb-3 line-clamp-2 font-cute">{task.description}</p>
                    )}
                    ...
                    <div className="space-y-2">
                      ...
                      {task.due_date && (
                        <div className={`flex items-center text-xs text-gray-500 ${isOverdue(task.due_date) ? 'text-rose-400' : ''}`}>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl w-full max-w-md border border-pink-100">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-semibold text-gray-800 font-cute">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 border border-pink-200 rounded-xl focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all resize-none font-cute"
                      rows="3"
